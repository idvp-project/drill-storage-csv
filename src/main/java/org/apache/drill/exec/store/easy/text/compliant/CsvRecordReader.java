...
public class CsvRecordReader extends AbstractRecordReader {
    ...
    private static final org.slf4j.Logger logger = org.slf4j.LoggerFactory.getLogger(CsvRecordReader.class);
    ...
    public CsvRecordReader(FileSplit split, DrillFileSystem dfs, TextParsingSettings settings, List<SchemaPath> columns) {
        this.split = split;
...
//    whitespaceBuffer = context.getManagedBuffer(WHITE_SPACE_BUFFER);
        ...
        try {
            ...
            String[] fieldNames = extractHeader(settings.isHeaderExtractionEnabled());
     ...
     */
    @SuppressWarnings("resource")
    private String[] extractHeader(boolean containsHeaders) throws SchemaChangeException, IOException, ExecutionSetupException {
        ...
        if (!containsHeaders) {
            HeaderBuilder gen = new HeaderBuilder();
            ...
            gen.finishRecord();
            fieldNames = gen.getHeaders();
        }

        // cleanup and set to skip the first line next time we read input
        reader.close();
        settings.setSkipFirstLine(containsHeaders);
